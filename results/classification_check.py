...
def ClassifierLocal(ICD,tp,pfa, TestType, pair, TP):
            ...
            for k in outlines_idx:
                grey_new = np.where((17 -Imc[start:end]) < value,17,Imc[start:end]+value)
                circleOutlines = plt.Circle((centro[k][0], centro[k][1]+Start), 15, color='r', fill=False)
                ax1.add_patch(circleOutlines)
                ax1.imshow(Imc[start:end], cmap = custom_cmap, interpolation='nearest')
            for j in detected_idx:
                circleDetected = plt.Circle((centro[j][0], centro[j][1]+Start), 15, color='k', fill=False)
                ax1.imshow(Imc[start:end], cmap = custom_cmap, interpolation='nearest')
                ax1.add_patch(circleDetected)
                
            plt.axis("off") 
            #path='/home/marcello-costa/workspace/kthSim/classDec/'
...
if __name__ == "__main__":
    ...
    if test_type == 'MSAR':
        path = '/home/marcello-costa/workspace/kthSim/Output/DataMSAR/'
        pfa=0.8 # due \ell=1
        
        
    elif test_type == 'MSARk':
        path = '/home/marcello-costa/workspace/kthSim/Output/DataMSARk/'
        pfa=2.2 # due \ell=2
        
    elif test_type == 'MSARkh':
        path = '/home/marcello-costa/workspace/kthSim/Output/DataMSARkh/'
        pfa=2.2 # due \ell=2
    ...
    if tp == 'S1':
        tp=sio.loadmat(path2+'S1.mat'); tp=tp['S1']; tp = np.fliplr(tp)
        
    elif tp == 'K1':
        tp=sio.loadmat(path2+'K1.mat'); tp=tp['K1']; tp = np.fliplr(tp)
        

    elif tp == 'AF1':
        tp=sio.loadmat(path2+'AF1.mat'); tp=tp['AF1']; tp = np.fliplr(tp)
    ...
    if ch == 1:
        ...
        [detected_targets, falseAlarm, mission_targets]=ClassifierLocal(ICD,tp,pfa, test_type, pair, TP)
